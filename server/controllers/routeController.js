...
const resolveDestination = async ({
  ...
  const fallbackLatitude = toNumber(destination_latitude);
  const fallbackLongitude = toNumber(destination_longitude);
  ...
  if (destination) {
    try {
      ...
      console.warn("ORS geocoding failed:", error.message);
  ...
  throw new Error(
    "Destination not found. Provide destination text or destination_latitude and destination_longitude."
  );
};
...
export const getSafeAlternativeRoutes = async (req, res) => {
  try {
    ...
    if (!destination && (destination_latitude == null || destination_longitude == null)) {
      return res.status(400).json({
        success: false,
        message:
          "Provide either destination text or destination_latitude and destination_longitude",
      });
    }
    ...
    const destinationData = await resolveDestination({
      destination,
      destination_latitude,
      destination_longitude,
    });
