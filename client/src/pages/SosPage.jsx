...
export default function SosPage() {
  ...
  for (let i = event.resultIndex; i < event.results.length; i++) {
    ...
    latestChunk += " " + result[0].transcript;
  }
  ...
  setLastTranscript(latestChunk);

  transcriptBufferRef.current =
    `${transcriptBufferRef.current} ${latestChunk}`.trim();

  if (transcriptBufferRef.current.length > 200) {
    transcriptBufferRef.current =
      transcriptBufferRef.current.slice(-200);
  }

  if (containsKeyword(transcriptBufferRef.current)) {
    const now = Date.now();
