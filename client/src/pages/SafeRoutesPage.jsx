...
function MapDestinationClickHandler({ onPickDestination }) {
  useMapEvents({
    click(event) {
      onPickDestination({
...
export default function SafeRoutesPage() {
  ...
  const [currentLocation, setCurrentLocation] = useState(null);
  const [destinationLocation, setDestinationLocation] = useState(null);
  const [pickedDestination, setPickedDestination] = useState(null);
  ...
  const handlePickDestinationOnMap = ({ latitude, longitude }) => {
    setPickedDestination({
      latitude,
      longitude,
      label: "Selected destination on map",
      source: "map_click",
    });

    setDestinationLatitude(latitude.toFixed(6));
    setDestinationLongitude(longitude.toFixed(6));
  ...
  const clearFallbackCoordinates = () => {
  ...
  const getRouteColor = (rank) => {
    if (rank === 1) return "#2a9d6e";
    if (rank === 2) return "#e08700";
    return "#97707f";
  };
  ...
  const handleFindRoutes = async () => {
    try {
      ...
      const fallbackLatitude = toNumberOrNull(destinationLatitude);
      const fallbackLongitude = toNumberOrNull(destinationLongitude);

      const hasFallbackCoordinates = isValidCoordinate(
        fallbackLatitude,
        fallbackLongitude
      );

      if (!destinationText && !hasFallbackCoordinates) {
        throw new Error(
          "Please enter a destination or tap the map to select destination coordinates."
        );
      }
      ...
      if (
        ...
        throw new Error("Invalid destination latitude or longitude.");
      }
      ...
      const data = await getSafeAlternativeRoutesApi({
        ...
        destination: destinationText || "Selected destination",

        destinationLatitude: hasFallbackCoordinates ? fallbackLatitude : null,
        destinationLongitude: hasFallbackCoordinates ? fallbackLongitude : null,
  ...
  const visibleDestination = destinationLocation || pickedDestination;
      ...
      <section className="safe-routes-shell">
          ...
          <div className="safe-routes-panel-content">
            <div className="brand-row">
              ...
              <div>
                <h1>Safe Routes</h1>
                <p>
                  Enter a destination, or tap the map if the place is not found.
                </p>
              </div>
            ...
            <div className="form-card">
              ...
              <p className="small-text">
                If ORS cannot recognize the place, tap the destination on the map
              ...
              {(destinationLatitude || destinationLongitude) && (
                <button
                  ...
                  onClick={clearFallbackCoordinates}
                >
              ...
              {visibleDestination && (
                <p className="small-text">
                  Destination: {visibleDestination.label || "Selected destination"}{" "}
                  ({Number(visibleDestination.latitude).toFixed(5)},{" "}
                  {Number(visibleDestination.longitude).toFixed(5)})
                </p>
              )}

              {destinationLocation?.source === "fallback_coordinates" && (
                <p className="small-text warning-text">
                  Used fallback destination coordinates because geocoding failed.
                </p>
              )}
            ...
            {routes.length > 0 && (
              <div className="route-list">
                ...
                <p className="route-note">Routes are ranked by safety.</p>
        ...
        <section className="safe-routes-map-panel">
          ...
          <MapContainer center={mapCenter} zoom={13} className="safe-routes-map">
            ...
            <MapDestinationClickHandler
              onPickDestination={handlePickDestinationOnMap}
            />
            ...
            {visibleDestination && (
              <Marker
                position={[
                  visibleDestination.latitude,
                  visibleDestination.longitude,
                ]}
                icon={destinationIcon}
              >
                <Popup>
                  {visibleDestination.label || "Selected destination"}
                </Popup>
              </Marker>
            )}
